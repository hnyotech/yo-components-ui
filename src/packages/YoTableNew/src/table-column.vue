<template>
  ...
  >
    ...
    <template slot-scope="scope">
      <el-input v-if="paramsStatus(tableColumnParams.type)==='input'"
                v-model="scope.row[tableColumnParams.key]"></el-input>
      <span v-else-if="paramsStatus(tableColumnParams.formatter)"
            v-html="tableColumnParams.formatter(scope.row, scope.column, scope.row[tableColumnParams.key], scope.$index)"></span>
      <span v-else v-html="scope.row[tableColumnParams.key]"></span>
    </template>
    <yo-table-column v-for="(item) in tableColumnParams.children" :key="RandomUniqueValue()"
